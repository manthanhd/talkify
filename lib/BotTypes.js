...
function MultiLineMessage(content) {
    ...
    return new Message('MultiLineMessage', content);
}

function Correspondence(id, message) {
    if(!(message instanceof Message)) throw new TypeError('message attribute must be of type Message');
    this.id = id;
    this.message = message;
}

function TrainingDocument(topic, text) {
    if(!topic || (typeof topic !== 'string' && !(topic instanceof String))) throw new TypeError('topic parameter is of invalid type. Must exist and be String.');
    if(!text || (typeof text !== 'string' && !(text instanceof String))) throw new TypeError('text parameter is of invalid type. Must exist and be String.');
